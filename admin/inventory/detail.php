        ...
        <div class="container mt-3">
            ...
            <table class="table table-bordered">
                ...
                <tbody id="myTable">
                    ...
                    $blood_group = $_GET['g'];
                    $rh = ($_GET['rh'] == 'plus' ? '+' : '-');
                    $sql = "SELECT bag_id , donation_id , center_id
                            FROM inventory
                            WHERE donation_id in (SELECT donation_id FROM donatio_result WHERE blood_group = '$blood_group' and rh = '$rh')
                                and center_id in (SELECT center_id FROM donation_center WHERE center_name = '$name')";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                        ...
                                        <td>" . $row['center_id'] . "</td>";
                                $link = 'http://localhost/EWUbloodbank/admin/inventory/transfer.php?bagid='.$row['bag_id'];

                                echo "<td><a href='$link'>Transfer</a></td>
                                    </tr>";
                            }
                        } else {
