...
<body>
    ...
    <div class="container">
        ...
        <h5 class="text-secoundary">Overview of total inventory</h5>
        <div class="container mt-3">
            <h2>All Inventory</h2>
            <p>Type Donation center in the search box to filtered out the expected result:</p>
            ...
            <table class="table table-bordered">
                ...
                <tbody id="myTable">
                    <?php
                    $sql = "SELECT center_name , blood_group , rh , COUNT(*) AS amount 
                            FROM (SELECT donation_id , center_name FROM inventory , donation_center WHERE inventory.center_id = donation_center.center_id) AS T1, 
                                (SELECT inventory.donation_id , blood_group , rh FROM inventory , donatio_result WHERE inventory.donation_id = donatio_result.donation_id) AS T2 
                            WHERE T1.donation_id = T2.donation_id 
                            GROUP BY center_name , blood_group , rh";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                        ...
                                        <td>" . $row['blood_group'].$row['rh'] . " </td>
                                        <td>" . $row['amount'] . "</td>";
                                $rh = ($row['rh'] == '+' ? 'plus' : 'minus');
                                $name = str_replace(' ' , '+' , $row['center_name']);
                                $link = 'http://localhost/EWUbloodbank/admin/inventory/detail.php?name=' . $name . '&g='.$row['blood_group'].'&rh='.$rh;
                                echo   "<td><a href='$link'>detail</a></td>
                                    </tr>";
                            }
                        } else {
