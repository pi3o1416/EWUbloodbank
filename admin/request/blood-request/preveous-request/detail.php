...
<header>
    ...
    <div class="container">
        ...
        <div class="container mt-3">
            <h2>All Preveous Request</h2>
            <p>Type Request id or hospital name in the search box to filtered out the expected result:</p>
            ...
            <table class="table table-bordered">
                ...
                <tbody id="myTable">
                    <?php
                    $sql = "SELECT req_id, req_h_id, req_center, req_blood_g, req_rh, req_amount, req_msg, req_status, req_date , h_name , center_name 
                            FROM hosp_bloodreq , hospital , donation_center 
                            WHERE h_id = req_h_id and req_center = center_id and hosp_bloodreq.req_status != 'pending' ";
                    $result = $connection->query($sql);
                    if ($result == true) {
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $rh = ($row['req_rh'] == '+' ? 'plus' : 'minus');
                                echo "<tr>
                                        <td>" . $row['req_id'] . "</td>
                                        <td>" . $row['h_name'] .  "</td>
                                        <td>" . $row['center_name'] . "</td>
                                        <td>" . $row['req_blood_g'] . "" . $row['req_rh'] . "</td>
                                        <td>"  . $row['req_amount'] . "</td>
                                        <td>"  . $row['req_date'] . "</td>
                                        <td>" . $row['req_msg'] . "</td>;";
                                if ($row['req_status'] == 'approved') {
                                    $link = "http://localhost/EWUbloodbank/admin/request/blood-request/preveous-request/detail.php?id=".$row['req_id'];
                                    echo    "<td>  <a href='$link'>". $row['req_status'] ."</a>  </td></tr>";
                                } else {
                                    echo    "<td>" . $row['req_status'] . "</td></tr>";
                                }
                            }
                        } else {
